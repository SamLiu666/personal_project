...
import pickle
import os
from visual import all_mask_kinds, position_embedding, transformer_attention_matrix,transformer_attention_line
"""矩阵中蓝色数值小，红色大。连线图中颜色越深越注意。Reference by https://github.com/MorvanZhou/NLP-Tutorials"""
...
class MultiHead(keras.layers.Layer):
    ...
    def scaled_dot_product_attention(self, q, k, v, mask=None):
        ...
        if mask is not None:
            score += mask * -1e9
        self.attention = tf.nn.softmax(score, axis=-1)   #(64, 4, 12, 12)
        # print("scaled_dot_product_attention: ", self.attention)                             # [n, h, q_step, step]
...
class Encoder(keras.layers.Layer):
    def __init__(self, n_head, model_dim, drop_rate, n_layer):
        ...
        self.ls = [EncoderLayer(n_head, model_dim, drop_rate) for _ in range(n_layer)]
...
if __name__ == '__main__':
    ...
    train(m, d, step=1001)   # 训练
    export_attention(m, d)  # 预测
    end = time.time()
